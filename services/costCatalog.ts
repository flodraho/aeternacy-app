
// services/costCatalog.ts

/**
 * A centralized catalog for the "Tokæn" cost of various AI-heavy features.
 * Tokæn are internal 'creative energy', not a cryptocurrency.
 */
export const TOKEN_COSTS = {
  /** Cost to generate a short, animated "Living Photo" from a header image. */
  HEADER_ANIMATION: 200,
  
  /** Cost to generate a full AI Video Reflection in the Video Studio. */
  AI_VIDEO_REFLECTION: 600,
  
  /** Cost to design and generate a downloadable Mægazine PDF. */
  MAGAZINE_ISSUE: 1200,
  
  /** Cost to begin one 15-minute Biografær interview session. */
  BIOGRAFER_SESSION: 400,
  
  /** Cost to analyze, declutter, and curate photos via Bulk Upload, per 1,000 photos. */
  BULK_UPLOAD_PER_1000: 900,
  
  /** Cost to generate a Legacy Letter and "seal" a Time Capsule. */
  TIME_CAPSULE_SEAL: 200
};
